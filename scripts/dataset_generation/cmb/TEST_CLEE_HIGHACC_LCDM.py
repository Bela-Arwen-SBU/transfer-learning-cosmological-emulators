#!/usr/bin/env python3
"""
Validation Script for EE CMB Training Data
===========================================
This script validates the EE data by:
...
Usage:
    python TEST_CLEE_HIGHACC_LCDM.py              # Both lightweight + CAMB (default)
    python TEST_CLEE_HIGHACC_LCDM.py --quick      # Lightweight only
    python TEST_CLEE_HIGHACC_LCDM.py --camb-only  # CAMB validation only
    python TEST_CLEE_HIGHACC_LCDM.py --n-samples 20 --seed 456
"""
import numpy as np
import sys
import argparse
...
def validate_data():
    ...
    print("="*70)
    print("CLEE Training Data Validation")
    print("="*70)
    if args.quick:
    ...
    data_file = 'CLEE_HIGHACC_LCDM.npy'
    param_file = 'COSMO_CLEE_HIGHACC_LCDM.dat'
    ...
    if run_lightweight:
        ...
        TT_EXPECTED_RANGE = (0, 50)  # EE is positive, smaller amplitude than TT
        ...
        for i, idx in enumerate(indices):
            ...
            if not (TT_EXPECTED_RANGE[0] <= sample_data.min() and sample_data.max() <= TT_EXPECTED_RANGE[1]):
                print(f"   ✗ Sample {i+1} (idx {idx}): EE values outside expected range")
                all_passed = False
        ...
        print(f"   EE range: [{data.min():.2e}, {data.max():.2e}] uK^2")
    ...
    if run_camb:
        ...
        for i, idx in enumerate(indices):
            ...
            try:
                ...
                cmb = theory.get_Cl()
                regenerated_ee = cmb["ee"][camb_ell_min:camb_ell_max]
                
                print(f"\n  Debug for sample {i+1}:")
                print(f"    Stored EE first 5 values: {stored_ee[:5]}")
                print(f"    Regenerated EE first 5 values: {regenerated_ee[:5]}")
                print(f"    Stored EE shape: {stored_ee.shape}")
                print(f"    Regenerated EE shape: {regenerated_ee.shape}")
                print(f"    Stored EE range: [{stored_ee.min():.2e}, {stored_ee.max():.2e}]")
                print(f"    Regenerated EE range: [{regenerated_ee.min():.2e}, {regenerated_ee.max():.2e}]")
                            
                # Compute chi^2
                diff = stored_ee - regenerated_ee
                chi2 = np.sum(diff**2)
