...
set -e
echo "========================================"
echo "BASELINE DATA SIZE SWEEP - Roman CS LCDM Taka"
echo "Started: $(date)"
echo "Host: $(hostname)"
echo "========================================"
...
OUTPUT_BASE="/home/grads/tmp/roman_cs_baseline/output_taka"
...
config['train_args']['${PROBE}']['extra_args']['file'] = ['${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.pt']
config['train_args']['${PROBE}']['extra_args']['extra'] = ['${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.h5']

with open('${TEMP_YAML}', 'w') as f:
    yaml.dump(config, f, default_flow_style=False, sort_keys=False)

print(f"✓ YAML written: ${TEMP_YAML}")
print(f"  Model: ${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.pt")
EOF
    ...
    if [ $TRAIN_EXIT_CODE -eq 0 ]; then
        ...
        if [ -f "${MODEL_DIR}/roman_cs_lcdm_taka_N${size}_metrics.txt" ]; then
            mv "${MODEL_DIR}/roman_cs_lcdm_taka_N${size}_metrics.txt" "${METRICS_DIR}/metrics_N${size}.txt"
        ...
        # Verify model files
        if [ -f "${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.pt" ] && \
           [ -f "${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.h5" ]; then
            MODEL_SIZE=$(du -h "${MODEL_DIR}/roman_cs_lcdm_taka_N${size}.pt" | cut -f1)
            echo "  ✓ Model saved (${MODEL_SIZE}): roman_cs_lcdm_taka_N${size}.pt"
